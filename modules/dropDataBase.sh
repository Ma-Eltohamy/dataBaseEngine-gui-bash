function dropDataBase() {
  ...
  local dataBaseName=$(zenity --entry --title="Drop Database" --text="Enter the name of the database you want to drop:" --entry-text="" --height=150 --width=400)
  ...
  if [[ $? -eq 0 ]]
  then
    rm -rf "$DB_PATH/$dataBaseName"  # Remove the entire database directory and its contents
